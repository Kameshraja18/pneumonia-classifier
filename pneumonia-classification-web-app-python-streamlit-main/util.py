...
def generate_gradcam_heatmap(model, image, class_index=None):
    """
    ...
    try:
        # Try to find the base model (e.g., MobileNet inside Sequential)
        base_model = None
        ...
        for layer in model.layers:
            if hasattr(layer, 'layers'):  # This is a nested model
                base_model = layer
                # Find last conv layer in the nested model
                for sub_layer in reversed(layer.layers):
                    if 'conv' in sub_layer.name.lower() and hasattr(sub_layer, 'output'):
                        try:
                            if len(sub_layer.output.shape) == 4:
        ...
        if base_model is None or last_conv_layer_name is None:
            # Fallback: use simple gradient visualization
            return generate_simple_heatmap(model, image)
        
        # Create gradient model
        last_conv_layer = base_model.get_layer(last_conv_layer_name)
        
        # Model that outputs the last conv layer and the final predictions
        ...
        # Get gradients of the class output with respect to the conv layer
        grads = tape.gradient(class_output, conv_outputs)
        
        # Global average pooling of gradients
        pooled_grads = tf.reduce_mean(grads, axis=(0, 1, 2))
        
        # Weight the conv layer outputs
    ...
    superimposed_img = create_superimposed_image(img, heatmap)
    
    return heatmap, superimposed_img


def generate_simple_heatmap(model, image):
    """
    Generate a simple activation-based heatmap when Grad-CAM fails.
    Uses the variance of activations across channels as a proxy for importance.
    """
    img = ImageOps.fit(image, (224, 224), Image.Resampling.LANCZOS)
    img_array = np.asarray(img)
    normalized_img = (img_array.astype(np.float32) / 127.5) - 1
    ...
    try:
        ...
        if activations is not None:
            # Use mean activation across channels
            heatmap = tf.reduce_mean(activations, axis=-1)[0]
            heatmap = tf.maximum(heatmap, 0)
            heatmap = heatmap / tf.math.reduce_max(heatmap)
            heatmap = heatmap.numpy()
        else:
            # Create a simple edge-based heatmap as ultimate fallback
...
def create_superimposed_image(original_img, heatmap, alpha=0.4):
    ...
    # Blue channel (high when heatmap is low)
    heatmap_colored[:, :, 2] = np.clip(1.5 - 3 * heatmap_resized, 0, 1)
    # Green channel (peaks in middle)
    heatmap_colored[:, :, 1] = np.clip(1.5 - np.abs(heatmap_resized - 0.5) * 4, 0, 1)
    # Red channel (high when heatmap is high)
    heatmap_colored[:, :, 0] = np.clip(3 * heatmap_resized - 1, 0, 1)
    
    # Convert original image to numpy
    original_array = np.array(original_img).astype(np.float32) / 255.0
    
    # Superimpose
    superimposed = original_array * (1 - alpha) + heatmap_colored * alpha
...
def create_pdf_report(class_name, confidence, filename, heatmap_image=None):
    """Create a PDF report for the analysis (placeholder - requires reportlab)."""
